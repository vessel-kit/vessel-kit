...
describe(".build", () => {
  ...
  test("known transport as scheme", () => {
    const result = ConnectionString.fromString(
      `https://mainnet.infura.io/non/ws`,
      {
        messaging: "openrpc",
        chain: "eip155",
      }
  ...
  test("invalid host", () => {
    expect(() => {
      ConnectionString.fromString(
        `eip155+openrpc+https://mainnet:infura.io/non/ws`
      );
    }).toThrow();
  });
});
...
describe(".isValid", () => {
  ...
  test("known transport", () => {
    expect(
      ConnectionString.isValid("https://mainnet.infura.io/non/ws")
    ).toBeTruthy();
  });
  test("invalid protocol", () => {
    expect(
      ConnectionString.isValid("pigeon://mainnet.infura.io/non/ws")
    ).toBeFalsy();
  });
  test("invalid url", () => {
    expect(
