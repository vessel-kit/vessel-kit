...
describe('AccountLinkHandler', () => {
  ...
  it('throws an error if genesis record has more than one owner', async () => {
    ...
    await expect(handler.makeGenesis({ doctype: 'account-link', owners: owners })).rejects.toThrow(/Exactly one owner/i)
  })
  ...
  it('makes signed record correctly', async () => {
    ...
    const record = await handler.makeRecord(state, 'did:3:bafyreiecedg6ipyvwdwycdjiakhj5hiuuutxlvywtkvckwvsnu6pjbwxae')

    expect(record).toEqual({ content: 'did:3:bafyreiecedg6ipyvwdwycdjiakhj5hiuuutxlvywtkvckwvsnu6pjbwxae', prev: FAKE_CID_1 })
  })
